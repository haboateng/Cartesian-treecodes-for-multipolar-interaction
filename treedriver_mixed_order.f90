...
! runtime parameters
      ...
      INTEGER :: numpars,order,maxparnode,mporder
      INTEGER :: shrink,treelevel,iflag
      REAL(KIND=r8) :: theta,t1,t2,rmsE,rmsF,temp,rmsFx,rmsFy,rmsFz 
...
! arrays for coordinates and charges and energy of target particles
      ...
      REAL(KIND=r8),ALLOCATABLE,DIMENSION(:) :: fxx,fyy,fzz,tfx,tfy,tfz !exact forces and treecode forces
      INTEGER,      ALLOCATABLE,DIMENSION(:) :: ord

! variables for potential energy computation

      REAL(KIND=r8) :: randi,tpeng,dpeng
...
! EXECUTABLE STATEMENTS
      ...
      ALLOCATE(tenergy(numpars),denergy(numpars),&
               fxx(numpars),fyy(numpars),fzz(numpars),&
               tfx(numpars),tfy(numpars),tfz(numpars),&
               ord(numpars),STAT=err)
      IF (err .NE. 0) THEN
...
! Read in the values for the exact energy at each target       
      ...
      DO i=1,numpars
         READ(84,17)denergy(i),fxx(i),fyy(i),fzz(i)
      END DO
...
! Calling main subroutine to approximate the energy
      ...
      CALL TREECODE(x,y,z,ord,mpole,numpars,mporder,mpdim,tenergy,tpeng,tfx,tfy,tfz,&
                    order,theta,shrink,maxparnode,timetree,treelevel,iflag)
...
! compute energy errors
         ...
         DO j=1,numpars
            ...
            WRITE(300,*)j
            WRITE(300,20)fxx(j),fyy(j),fzz(j)
            WRITE(300,20)tfx(j),tfy(j),tfz(j)
         END DO

        DO j=1,numpars
            temp=ABS( (fxx(j)-tfx(j))/fxx(j) )+ABS((fyy(j)-tfy(j))/fyy(j))+ABS( (fzz(j)-tfz(j))/fzz(j))
            IF (temp .GT. relinferr)THEN
              relinferr=temp
              k=j
            END IF
         ...
         rmsF=SQRT( ( DOT_PRODUCT(fxx-tfx,fxx-tfx) + DOT_PRODUCT(fyy-tfy,fyy-tfy) + &
                      DOT_PRODUCT(fzz-tfz,fzz-tfz) ) / &
                    ( DOT_PRODUCT(fxx,fxx) + DOT_PRODUCT(fyy,fyy) + DOT_PRODUCT(fzz,fzz) ) )

         rmsFx=SQRT(DOT_PRODUCT(fxx-tfx,fxx-tfx)/DOT_PRODUCT(fxx,fxx))
         rmsFy=SQRT(DOT_PRODUCT(fyy-tfy,fyy-tfy)/DOT_PRODUCT(fyy,fyy))
         rmsFz=SQRT(DOT_PRODUCT(fzz-tfz,fzz-tfz)/ DOT_PRODUCT(fzz,fzz))
...
! output errors to standard out
         ...
         WRITE(6,*) ' '
         WRITE(6,13)rmsE
         WRITE(6,*)'max location = ',i,k
         
         write(85,15)numpars,iflag,maxparnode,treelevel,order,theta
         write(85,16)ABS((tpeng-dpeng)/dpeng),rmsE,rmsF,timedirect,timetree
!         WRITE(85,20)rmsFx,rmsFy,rmsFz

      CLOSE(unit=85)

      DEALLOCATE(tenergy,denergy,fxx,fyy,fzz,tfx,tfy,tfz)
 13   FORMAT(2X,E24.16,2X,E24.16)
 14   FORMAT(E24.16,1X,E24.16)
 15   FORMAT(I8,2X,I4,2X,I4,2X,I4,2X,I2,2X,F12.8)
 16   FORMAT(5(2X,E24.16)) 
 17   FORMAT(12X,4(2X,E24.16))
 20   FORMAT(3(2X,E24.16))
      STOP
