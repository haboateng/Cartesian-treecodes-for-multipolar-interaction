...
! local variables
      ...
      INTEGER, DIMENSION(8,2) :: ind
      REAL(KIND=r8), DIMENSION(6,8) :: xyzmms
      INTEGER :: i,j,err,loclev,numposchild
      REAL(KIND=r8), DIMENSION(6) ::  lxyzmm
...
!
      ...
      INTEGER,INTENT(IN) :: numpars,mpdim,idnode,mxnode
      TYPE(tnode),POINTER :: p  
...
! local variables
      ...
      INTEGER :: i,j,atm0,atm1
      REAL(KIND=r8) :: penglocal,peng,xtemp,mptemp(1:mpdim)


!split targets over processors
      atm0=(idnode*numpars)/mxnode+1
      atm1=((idnode+1)*numpars)/mxnode
      tpeng=0.0_r8
      DO i=atm0,atm1
         peng=0.0_r8
         tarpos(1)=x(i)
...
! local variables
      ...
      REAL(KIND=r8) :: tx,ty,tz,distsq,penglocal,k2f,k3f,fac
...
!!!!!!!!!!!!!!!
      SUBROUTINE TREECODE(x,y,z,mpole,numpars,mpdim, &
                      ...
                      maxparnode,treelevel,iflag,idnode,mxnode) 
...
!=====================================================================
      ...
      INTEGER,INTENT(IN) :: numpars,mpdim,order,shrink,maxparnode
      INTEGER,INTENT(IN) :: idnode,mxnode,treelevel,iflag
      REAL(KIND=r8),DIMENSION(numpars),INTENT(INOUT) :: x,y,z
      REAL(KIND=r8),DIMENSION(numpars,1:mpdim),INTENT(INOUT) :: mpole
      REAL(KIND=r8),DIMENSION(numpars),INTENT(OUT) :: tEn
      REAL(KIND=r8),INTENT(IN) :: theta
      REAL(KIND=r8),INTENT(OUT) :: tpeng
...
! and setup global variables.  
      ...
      CALL SETUP(x,y,z,numpars,order,theta,xyzminmax)
