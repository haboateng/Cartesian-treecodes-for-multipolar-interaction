...
!!!!!!!!!!!!!!!!!!!!!!!!
      SUBROUTINE CP_TREECODE(p,x,y,z,mpole,tpeng,EnP,tfx,tfy,tfz,numpars,mpdim)
      IMPLICIT NONE
!
...
!=========================================================================
         DO j=1,p%num_children
            CALL COMPUTE_CP1(p%child(j)%p_to_tnode,EnP, &
                           tfx,tfy,tfz,x,y,z,mpole,mpdim,numpars)
         END DO
         
      END DO

      CALL COMPUTE_CP2(p,mpole,x,y,z,tfx,tfy,tfz,EnP,mpdim,numpars)
...
!!!!!!!!!!!!!!
      RECURSIVE SUBROUTINE COMPUTE_CP1(p,EnP,tfx,tfy,tfz,x,y,z,mpole,mpdim,arrdim)
      IMPLICIT NONE
...
! box use the expansion for the approximation.
      IF ((p%sqradius .LT. distsq*thetasq) .AND. &
         ...
         IF (p%exist_ms .EQ. 0) THEN
             ALLOCATE(p%ms(0:torder,0:torder,0:torder),&
             p%fms(1:mpdim,0:torder,0:torder,0:torder),STAT=err)
             IF (err .NE. 0) THEN
...
! local variables
        ...
        INTEGER :: i,nn,j,k1,k2,k3,torder1
        ...
        IF(ap%exist_ms==1)THEN
         ...
         DO i=ap%ibeg,ap%iend
           ...
           fx=0.0_r8; fy=0.0_r8; fz=rt*dot_product(imp,ap%fms(:,0,0,torder))
           peng=ap%ms(0,0,torder)           

           DO k3=torder1,0,-1

              ty  = ap%ms(0,torder-k3,k3)

              tbk = dot_product(imp,ap%fms(:,0,torder-k3,k3))
              rk3 = REAL(k3,KIND=r8)
              xf2 = 0.0_r8; yf2=(rt-rk3)*tbk; zf2=rk3*tbk

              DO k2=torder1-k3,0,-1

                 tx  = ap%ms(torder-k3-k2,k2,k3)
 
                 tbk = dot_product(imp,ap%fms(:,torder-k2-k3,torder-k3,k3)) 
                 rk2 = REAL(k2,KIND=r8)
                 ...
                 DO k1=torder1-k3-k2,0,-1
                   ...
                   tbk = dot_product(imp,ap%fms(:,k1,k2,k3))
                   rk1 = REAL(k1,KIND=r8)
...
! target: n =(0,0,2)
      ...
      SUBROUTINE COMP_DIRECT(EnP,tfx,tfy,tfz,ibeg,iend,x,y,z,mpole,mpdim,arrdim)
      IMPLICIT NONE
!
! COMP_DIRECT directly computes the interaction between the current 
! target cluster and the current source particle

      INTEGER,INTENT(IN) :: ibeg,iend,mpdim,arrdim
      REAL(KIND=r8),DIMENSION(arrdim),INTENT(INOUT) :: EnP,tfx,tfy,tfz
      REAL(KIND=r8),DIMENSION(arrdim),INTENT(IN) :: x,y,z
      REAL(KIND=r8),DIMENSION(arrdim,1:mpdim),INTENT(IN) :: mpole
...
! local variables
      ...
      IF (targid >= ibeg .AND. targid <= iend) THEN
         ...
         CALL DIRECT_EVAL(EnP,tfx,tfy,tfz,ibeg,targid-1,x,y,z,mpole,mpdim,arrdim)
         CALL DIRECT_EVAL(EnP,tfx,tfy,tfz,targid+1,iend,x,y,z,mpole,mpdim,arrdim)

      ELSE

         CALL DIRECT_EVAL(EnP,tfx,tfy,tfz,ibeg,iend,x,y,z,mpole,mpdim,arrdim)
      ...
      SUBROUTINE DIRECT_EVAL(EnP,tfx,tfy,tfz,ibeg,iend,x,y,z,mpole,mpdim,arrdim)
      IMPLICIT NONE
!
! COMP_DIRECT directly computes the interaction between the current
! target cluster and the current source particle
...
! local variables
      ...
      IF (p%exist_ms .EQ. 1) THEN
         DEALLOCATE(p%ms,p%fms,STAT=err)
         IF (err .NE. 0) THEN
...
!Call driver routine for particle cluster
      ...
      CALL CP_TREECODE(troot,x,y,z,mpole,tpeng,tEn,tfx,tfy,tfz,numpars,mpdim) 
