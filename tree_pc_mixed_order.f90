...
!!!!!!!!!!!!!!!!!!!!!!!!
      SUBROUTINE PC_TREECODE(p,x,y,z,ord,mpole,tpeng,EnP,tfx,tfy,tfz,&
                             numpars,mpdim,mporder)
      IMPLICIT NONE
!
...
!
      ...
      INTEGER,INTENT(IN) :: numpars,mpdim,mporder
      TYPE(tnode),POINTER :: p
      ...
      DO i=1,numpars
         ...
         DO j=1,p%num_children
            CALL COMPUTE_PC(p%child(j)%p_to_tnode,penglocal, &
                           flcx,flcy,flcz,x,y,z,ord,mpole,&
                           numpars,mpdim,mporder)
            peng=peng+penglocal
            fx=fx+flcx
            fy=fy+flcy
...
!!!!!!!!!!!!!!
      RECURSIVE SUBROUTINE COMPUTE_PC(p,peng,fx,fy,fz,x,y,z,ord,mpole,&
                                      arrdim,mpdim,mporder)
...
!
      ...
      INTEGER,INTENT(IN) :: arrdim,mpdim,mporder
      TYPE(tnode),POINTER :: p
      ...
      IF ((p%sqradius .LT. distsq*thetasq) .AND. &
         ...
         IF (p%exist_ms .EQ. 0) THEN
             ...
             IF (err .NE. 0) THEN
                ...
                STOP
             END IF
             CALL COMP_MS(p,x,y,z,ord,mpole,arrdim,mpdim)
             p%exist_ms=1
         END IF
         penglocal=0.0_r8
...
!
         IF (p%num_children .EQ. 0) THEN
            CALL COMP_DIRECT(penglocal,p%ibeg,p%iend, &
                              flcx,flcy,flcz,x,y,z,ord,mpole,arrdim,mpdim)
            peng=penglocal
            fx=flcx; fy=flcy; fz=flcz
         ELSE
            DO i=1,p%num_children
               CALL COMPUTE_PC(p%child(i)%p_to_tnode,penglocal, &
                              flcx,flcy,flcz,x,y,z,ord,mpole,arrdim,mpdim,mporder)
               peng=peng+penglocal
               fx=fx+flcx; fy=fy+flcy; fz=fz+flcz
            END DO
         END IF
...
!!!!!!!!!!!!!!!
      SUBROUTINE COMP_MS(p,x,y,z,ord,mpole,arrdim,mpdim)
      IMPLICIT NONE
!
...
! local variables
      ...
      DO j=p%ibeg,p%iend
         ...
         ELSEIF (ord(j) .EQ. 1) THEN

            jmp=mpole(j,:)
            m1=mpole(j,1); m2=mpole(j,2); m3=mpole(j,3); m4=mpole(j,4) 
...
!!!!!!!!!!!!!!!!!!!!!!!!
      SUBROUTINE COMP_DIRECT(peng,ibeg,iend,fx,fy,fz,x,y,z,ord,mpole,arrdim,mpdim)
      IMPLICIT NONE
!
      ...
      fx=0.0_r8; fy=0.0_r8; fz=0.0_r8
       SELECT CASE (tarord)
         CASE (0) 
           ...
           CALL COMPUTE_ORD_ZERO(x,y,z,ord,mpole,ibeg,iend,mpdim,&
                          fx,fy,fz,peng,arrdim) 

         CASE (1)
           
           CALL COMPUTE_ORD_ONE(x,y,z,ord,mpole,ibeg,iend,mpdim,&
                          fx,fy,fz,peng,arrdim)

         CASE (2)
          
           CALL COMPUTE_ORD_TWO(x,y,z,ord,mpole,ibeg,iend,mpdim,&
                          fx,fy,fz,peng,arrdim)

         CASE DEFAULT

             WRITE(*,*)'The order ',tarord,' is out of range'
             STOP
        END SELECT           
...
!!!!!!!!!!!!!!!!!
      SUBROUTINE COMPUTE_ORD_ZERO(x,y,z,ord,mpole,ibeg,iend,mpdim,&
                            fx,fy,fz,peng,numpars)
      
      IMPLICIT NONE

      INTEGER,PARAMETER :: r8=SELECTED_REAL_KIND(12)
      INTEGER,INTENT(IN) :: ibeg,iend,mpdim,numpars
      REAL(KIND=r8),DIMENSION(numpars),INTENT(IN) :: x,y,z
...
! potential
              ...
              peng=peng+rr3*(jmp2*xx+jmp3*yy+jmp4*zz)
...
!!!!!!!!!!!!!!!!!
      SUBROUTINE COMPUTE_ORD_ONE(x,y,z,ord,mpole,ibeg,iend,mpdim,&
                            fx,fy,fz,peng,numpars)
...
!!!!!!!!!!!!!!!!!
      SUBROUTINE COMPUTE_ORD_TWO(x,y,z,ord,mpole,ibeg,iend,mpdim,&
                            fx,fy,fz,peng,numpars)
...
!Call driver routine for particle cluster
      ...
      CALL PC_TREECODE(troot,x,y,z,ord,mpole,tpeng,tEn,tfx,tfy,tfz,&
                       numpars,mpdim,mporder)
